...
def write_line_element(element_text, x_box, y_line):
...
def assemble_image(hourly_df, daily_df, base_dir):
    ...
    for i, row in hourly_df.iterrows():
        ...
        write_line_element(temperature_text, 428, y_line)

        pp_text = f"{round(pp)}%"
        write_line_element(pp_text, 650, y_line)


    # Save
    img.save("weather_info.jpg")
